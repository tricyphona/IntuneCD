...
from src.IntuneCD.backup.Intune.WindowsFeatureUpdates import (
    WindowsFeatureUpdatesBackupModule,
)
from src.IntuneCD.intunecdlib.GraphModule import GraphModule


class TestWindowsFeatureUpdatesBackupModule(unittest.TestCase):
    """Tests for the WindowsFeatureUpdatesBackupModule class."""

    def setUp(self):
        self.module = WindowsFeatureUpdatesBackupModule()

    @patch.object(GraphModule, "make_graph_request")
    @patch.object(WindowsFeatureUpdatesBackupModule, "process_data")
    ...
    @patch.object(GraphModule, "make_graph_request")
    @patch.object(WindowsFeatureUpdatesBackupModule, "log")
    def test_main_logs_exception_graph_data(self, mock_log, mock_make_graph_request):
        ...
        mock_log.assert_called_with(
            tag="error",
            msg=f"Error getting Windows Feature Update data from {self.module.endpoint + self.module.CONFIG_ENDPOINT}: Test exception",
        )
    ...
    def test_main_logs_exception_process_data(
        ...
        mock_log.assert_called_with(
            tag="error",
            msg="Error processing Windows Feature Update data: Test exception",
        )
